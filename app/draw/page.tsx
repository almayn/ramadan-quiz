...
export default function Draw() {
  ...
  const [winnerSourceSnap, setWinnerSourceSnap] = useState('');
const [showSourceSnap, setShowSourceSnap] = useState(false);
...
// ✅ جلب جميع المشاركين وغير الفائزين
let query = supabase
  .from('participants')
  .select('subscription_number, name, city, source_snap')
  .eq('question_id', questionId);
...
if (!participants || participants.length === 0) {
  ...
  return;
      ...
      const { error: insertError } = await supabase.from('winners').insert([
        {
          ...
          source_snap: winningData.source_snap,
      ...
      setWinnerSourceSnap(winningData.source_snap || '');
      playCheer();
      setTimeout(() => {
    ...
    <div className="min-h-screen flex items-center justify-center bg-gray-100">
      <div className="max-w-sm w-full mx-auto p-4 bg-white shadow-xl rounded-lg text-center relative overflow-hidden">
        ...
        <div className="bg-blue-600 text-white rounded-t-lg py-2 mb-4">
          <h2 className="text-2xl font-bold">مسابقة نجوم ينبع</h2>
        </div>
        ...
        <div className="bg-gray-50 p-4 rounded-lg shadow-inner">
          ...
          <p className="text-lg text-gray-800 font-bold mt-2">
            {showCongrats ? ` ألف مبروك! ${winnerName}` : 'مسابقة نجوم'}
          </p>
          {showCongrats && winnerSourceSnap && (
  <div className="mt-2">
    <button
      onClick={() => setShowSourceSnap(v => !v)}
      className="text-sm text-blue-600 underline"
    >
    {showSourceSnap ? 'نجوم ينبع ' : 'المصدر'}

    </button>

    {showSourceSnap && (
      <p className="mt-1 text-sm text-gray-700">
       : <span className="font-bold">{winnerSourceSnap}</span>
      </p>
    )}
  </div>
)}
