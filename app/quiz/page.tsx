...
export default function QuizPage() {
  ...
  const [sourceSnap, setSourceSnap] = useState('');
...
if (!name || !city || !selectedAnswer || !sourceSnap) {
...
return (
  ...
  <Image
    ...
    width={160}
    height={160}
    className="w-40 h-40 mx-auto rounded-lg border-4 border-yellow-400 shadow-lg"
  />
</div>
        ...
        {!showThankYouMessage ? (
          ...
          <p className="text-green-600 font-bold text-lg">
            شكراً لمشاركتك.. تابعنا  لمعرفة النتيجة
          </p>
...
{/* حقل الجهة (السناب اللي عرفه على المسابقة) */}
<div className="flex items-center gap-2">
  <label
    htmlFor="sourceSnap"
    className="text-md font-semibold text-gray-700 bg-gray-200 border border-gray-300 rounded px-2 py-1"
  ...
  <select
    id="sourceSnap"
    value={sourceSnap}
    onChange={(e) => setSourceSnap(e.target.value)}
    className="flex-1 py-2 px-4 border border-gray-300 rounded focus:outline-none focus:ring-2 focus:ring-blue-500 bg-white"
  >
   <option value="">اختر من القائمة</option>
    <option value="سناب الماس">سناب الماس</option>
    <option value="الخبر اليقين"> الخبر اليقين</option>
    <option value="لافي الرفاعي"> لافي الرفاعي</option>
    <option value=" لايف أبو جسار"> لايف ابو جسار</option>
    <option value="نجم الساحل">نجم الساحل</option>
    <option value="بندر شو">بندر شو</option>
    <option value="عيسى السناني">عيسى السناني</option>
